...
func TestBrowserProfileCreatePathsDoNotApplyGlobalExtensionWithoutDistribution(t *testing.T) {
	root := t.TempDir()
	app := NewApp(root)
	...
	createdBatch, err := app.BrowserProfileBatchCreate("批量", 1, 2, BrowserProfileInput{})
	if err != nil {
		t.Fatal(err)
	}
	for _, created := range createdBatch {
		if hasExtensionDirInLaunchArgs(created.LaunchArgs, extDir) {
			t.Fatalf("batch-created profile must wait for an explicit distribution action: %#v", created.LaunchArgs)
		}
		prefsPath := filepath.Join(app.browserMgr.ResolveUserDataDir(created), "Default", "Preferences")
		if prefs, err := os.ReadFile(prefsPath); err == nil && strings.Contains(string(prefs), `"developer_mode": true`) {
			t.Fatalf("batch creation unexpectedly changed extension preferences: %s", prefs)
		}
	}
}

...
class PackagingScriptsTest(unittest.TestCase):
    ...
    def test_sync_window_collection_is_action_driven(self):
        ...
        self.assertIn("void loadProfiles(false)", page)
        ...
        self.assertNotIn("discoverBoostBrowserProcessesCached", backend)
        self.assertIn("requestMainSyncCollection(true)", backend)
        self.assertIn("collectMainClientSyncProfiles", backend)
        self.assertNotIn("readBrowserRuntimeSnapshot", backend)
        self.assertNotIn("PrepareWindowSyncRuntimeSnapshot", main_helpers)
        self.assertIn("resolveLiveMainEnvironmentFrame", backend)
        self.assertNotIn("findProcessTreeWindow(profile.Pid)", backend)
        self.assertNotIn("findProcessWindow(profile.Pid)", backend)
    ...
    def test_profile_delete_removes_owned_data_and_sync_accepts_arranged_windows(self):
        ...
        self.assertNotIn("discoverBoostBrowserProcessesCached", sync_api)
        self.assertNotIn("readBrowserDebugPortFile", sync_api)
        self.assertNotIn("profileByDataDir", sync_api)
        self.assertIn("a.syncCollection.snapshot()", sync_api)

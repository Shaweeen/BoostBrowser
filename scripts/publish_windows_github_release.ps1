...
if ($Version -eq '1.7.95' -and [string]::IsNullOrWhiteSpace($ApprovedGrowthReason)) {
    $ApprovedGrowthReason = 'v1.7.95 extension recovery from v1.7.83 (path heal + proxy CRX + assign stability + publish health-base fix)'
}
$healthArgs = @(
    '-NoProfile',
    '-ExecutionPolicy',
